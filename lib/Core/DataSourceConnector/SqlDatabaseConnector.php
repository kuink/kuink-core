...
class SqlDatabaseConnector extends \Kuink\Core\DataSourceConnector
{
    ...
    private function xsql($instruction, $params, $count = false)
    {
        ...
        if ($acl == 'true') {
            ...
            $aclPermissions = str_replace(' ', "", $aclPermissions);
            $aclPermissions = "'" . $aclPermissions . "'";
            $limit = 1000; // hardcoded only to define the variable. The query will give an error if this is not defined
    ...
    private function getExpandedAttribute($attr, $nodeManager)
    {
        ...
        if ($attr ['domain'] != '') {
            ...
            foreach ($domainAttributes as $key => $value) {
                if ($key != 'name' && $key != 'doc') {
                    $attr [$key] = isset ($domainAttributes [$key]) ? ( string )$domainAttributes [$key] : ( string )$value;
                }
            }
        }
        ...
        if ($typeConverted) {
            ...
            throw new PhysicalTypeNotFound (__CLASS__, $typeConverted, ( string )$attr ['name'], ( string )$attr ['type']);
        }
    ...
    private function getEntityWithChanges($nodeManager, $entity, $onlyMultilangAttributes = false, $drop = false)
    {
        ...
        if (count($physical) == 0) {
            ...
            foreach ($physical as $physicalAttr) {
                ...
                if (isset ($entAttr)) {
                    ...
                    $attr ['pk'] = isset($domain ['pk']) ? $domain ['pk'] : '';
                    $attr ['pk'] = isset($entAttr ['pk']) ? $entAttr ['pk'] : $attr['pk'];
                    $attr ['autonumber'] = isset($domain ['autonumber']) ? $domain ['autonumber'] : '';
                    $attr ['autonumber'] = isset($entAttr ['autonumber']) ? $entAttr ['autonumber'] : $attr['autonumber'];
                    $attr ['required'] = isset($domain ['required']) ? $domain ['required'] : '';
                    $attr ['required'] = isset($entAttr ['required']) ? $entAttr ['required'] : $attr['required'];
                    $attr ['foreign'] = isset($domain ['foreign']) ? $domain ['foreign'] : '';
                    $attr ['foreign'] = isset($entAttr ['foreign']) ? $entAttr ['foreign'] : $attr['foreign'];
                    $attr ['multilang'] = isset($domain ['multilang']) ? $domain ['multilang'] : '';
                    $attr ['multilang'] = isset($entAttr ['multilang']) ? $entAttr ['multilang'] : $attr['multilang'];
                    $attr ['refentity'] = isset($domain ['refentity']) ? $domain ['refentity'] : '';
                    $attr ['refentity'] = isset($entAttr ['refentity']) ? $entAttr ['refentity'] : $attr['refentity'];
                    $attr ['refattr'] = isset($domain ['refattr']) ? $domain ['refattr'] : '';
                    $attr ['refattr'] = isset($entAttr ['refattr']) ? $entAttr ['refattr'] : $attr['refattr'];
                    $attr ['type'] = isset($domain ['type']) ? $domain ['type'] : '';
                    $attr ['type'] = isset($entAttr ['type']) ? $entAttr ['type'] : $attr['type'];
                    $attr ['size'] = isset($domain ['size']) ? $domain ['size'] : '';
                    $attr ['size'] = isset($entAttr ['size']) ? $entAttr ['size'] : $attr['size'];
                    $attr ['default'] = isset($domain ['default']) ? $domain ['default'] : '';
                    $attr ['default'] = isset($entAttr ['default']) ? $entAttr ['default'] : $attr['default'];
        ...
        foreach ($entAttrs as $entAttr) {
            ...
            if (!$phFound) {
                ...
                $attr ['autonumber'] = isset($domain ['autonumber']) ? $domain ['autonumber'] : '';
                $attr ['autonumber'] = isset($entAttr ['autonumber']) ? $entAttr ['autonumber'] : $attr['autonumber'];
                $attr ['required'] = isset($domain ['required']) ? $domain ['required'] : 'false';
                $attr ['required'] = isset($entAttr ['required']) ? $entAttr ['required'] : $attr['required'];
                $attr ['foreign'] = isset($domain ['foreign']) ? $domain ['foreign'] : '';
                $attr ['foreign'] = isset($entAttr ['foreign']) ? $entAttr ['foreign'] : $attr['foreign'];
                $attr ['multilang'] = isset($domain ['multilang']) ? $domain ['multilang'] : '';
                ...
                if ($refEntity != '') {
                    ...
                    if (strpos($refEntity, ',')) {
                        ...
                        if (count($splitedName) != 3)
                            throw new InvalidName(__CLASS__, $splitedName);
