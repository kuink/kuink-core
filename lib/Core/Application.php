...
class Application {
	...
	function __construct($name, $lang, $config, $core) {
		...
		DataSourceManager::setupFrameworkDS ( $this );
		...
		if (Configuration::getInstance()->kuink->use_new_data_access_infrastructure)
			...
			DataSourceManager::setupCompanyDataSources ( ProcessOrchestrator::getCompany () );
		...
		if (Configuration::getInstance()->kuink->use_new_data_access_infrastructure)
			// Setup framework dataSources
			DataSourceManager::setupApplicationDS ( $this );
	...
	private function loadFrameworkDefinition() {
		$this->fwXmlDefinition = simplexml_load_file ( Configuration::getInstance()->paths->apps . '/framework/framework.xml' );
	}
	...
	private function loadInstanceConfig($instanceConfigRaw) {
		...
		if (trim ( $instanceConfigRaw ) != '') {
			...
			// TODO joao.patricio Don't really know if this should be read at the session variable
			$current_user_id = Configuration::getInstance()->defaults->user->id ?? 0;
	...
	function makeMenuItems($flowMenu) {
		...
		foreach ($flowMenu as $node) {
			...
			if ($hasRole) {
				...
				$href = $configuration->web->www_root . '/view.php?id=' . $qstrId . '&idcontext=' . $_GET ['idcontext'] . '&startuc=' . $node ['startuc'] . '&startnode=' . $node ['startnode'] . '&event=' . $node ['event'] . '&trace=' . $qstrTrace;
				$hrefNoContext = ($configuration->kuink->allow_multiple_contexts)
                    ? $configuration->web->www_root . '/view.php?id=' . $qstrId . '&idcontext=' . uniqid () . '&startuc=' . $node ['startuc'] . '&startnode=' . $node ['startnode'] . '&event=' . $node ['event'] . '&trace=' . $qstrTrace
                    : '';
	...
	private function getNodeConfiguration($node) {
		...
		$baseUrl = Configuration::getInstance()->web->www_root . '/view.php?id=' . $qstrId;
		$baseUrlParams = array (
