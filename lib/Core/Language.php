...
class Language {
	...
	static private function getTranslation($xPathElement, $xPathAttribute, $identifier, $app_name='framework', $params=null){
		...
		$paramsList = !empty($params) ? ' ('.implode(',', $params).')' : '';
	...
	static private function loadLaguageFile($app_name, $lang) {
		...
		$langfilename = Configuration::getInstance()->paths->apps . '/' . $appBase . '/' . $app_name . '/lang/' . $userlang_final . '.xml';
		
		if (! file_exists ( $langfilename ))
			$langfilename = Configuration::getInstance()->paths->apps . '/' . $appBase . '/' . $app_name . '/lang/pt.xml';
