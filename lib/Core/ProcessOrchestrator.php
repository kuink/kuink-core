...
class ProcessOrchestrator
{
    ...
    static function addContext($baseApplicationName, $contextId = null)
    {
        ...
        if ($currentContext == null) {
            if (!$configuration->kuink->allow_multiple_contexts)
                self::clearContexts();
        }

        $numContexts = isset($_SESSION ['KUINK_CONTEXT']) ? count($_SESSION ['KUINK_CONTEXT'] ['CONTEXTS']) : 0;
        if ($numContexts < $configuration->kuink->max_process_orchestrator_contexts) {
    ...
    static function addNode($node, $contextId = null)
    {
        ...
        if ($found != -1) {
            ...
            $newNode->idUser = (Configuration::getInstance()->defaults->user->id) ?? 0;
            $qstrId = isset($_GET [QueryStringParam::ID]) ? $_GET [QueryStringParam::ID] : '';
            $newNode->url = Configuration::getInstance()->web->www_root. '/view.php?id=' . $qstrId . '&' . QueryStringParam::ID_CONTEXT . '=' . $_GET [QueryStringParam::ID_CONTEXT] . '&' . QueryStringParam::NODE_GUID . '=' . $newNode->nodeGuid;
    ...
    static function processEvent($roles, $flow)
    {
        ...
        $flowFile = Configuration::getInstance()->paths->apps . '/' . $appBase . '/' . $flow->application . '/process/' . $flow->process . '/process.xml';
