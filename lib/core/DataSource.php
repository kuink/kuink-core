...
class DataSource
{
	...
	function __construct( $screen_domobject, $datasourcename, $appname, $processname, $databasename='', $databaseApp='' )
	{
		...
		$this->datasourcexml_domobject = simplexml_load_file($KUINK_CFG->appRoot.'apps/'.$appBase.'/'.$methodappname.'/process/'.$methodprocessname.'/dataaccess/'.$methodname.'.xml', 'SimpleXMLElement', LIBXML_NOCDATA);
	...
	{
	   $utils = new \UtilsLib();
		$records = $utils->xmlToSet( array(0 => $instruction->asXML()));
