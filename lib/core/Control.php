...
abstract class Control
{
	...
	/**
	 * Base Contructor
	 * @param $nodeconfiguration - node configuration where the control is
	 * @param $msg_manager - this allow this control to set messages to the user
	 * @param $xml_definition - contains a pointer to an xml object containing the control
	 */
	 ...
	 * @param $params
	 * [0] - (string) key to set the property
	...
	function setProperty( $params ) {
		...
		} else throw new \Exception($this->type.'->'.$this->name.'. setProperty: invalid number of parameters.');
	 ...
	 * @param (int) $num - index to return the value
	 * @param (bool) $mandatory - is this parameter mandatory?
	 * @param $default - default value to return if not mandatory set
	 * @throws Exception
	 */
	 ...
	 * @param xml $xml - the element where to get the property. If null, get the property directly from xml_definition
    ...
    function loadDataSource($datasourcename, $bindid, $bindvalue)
    {
    	...
    	if (!isset($this->datasources[$datasourcename]))
    	{
	    	...
	    	if ($pos === 0 && !$this->datasources[$datasourcename])
	    	{
	    		...
	    		$pars=array('table'=>$table, 'fields'=>$fields);
	    	...
	    	if ($pos === 0 && !$this->datasources[$datasourcename])
	    	{
    			...
    			$result = $runtime->execute($parts[3], null);
