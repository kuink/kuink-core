...
//var_dump($currentNode);
            ...
            $personData = array('id_person'=>$kuinkUser['id'], 'id_company'=>ProcessOrchestrator::getCompany());
...
class Runtime
{
     ...
     * @param $roles Node configuration roles
     */
	...
	function action_execute(&$nodeconfiguration, $nodexml, $action_xmlnode, $actionname,  &$variables)
	{
		...
		if ($log_level == 'action')
		{
			...
			//'timestamp'=> $now
			//var_dump($nodeconfiguration);
			$pars=array( 'table'=>'log' ,'id_user'=>$KUINK_BRIDGE_CFG->auth->user->id , 'type'=>$log_level, 'application'=>$nodeconfiguration[NodeConfKey::APPLICATION], 'process'=>$nodeconfiguration[NodeConfKey::PROCESS], 'node'=>$nodeconfiguration[NodeConfKey::NODE], 'action'=>$actionname, 'log_key'=>$key, 'timestamp'=>time() );
                ...
                if ($uielem_type == 'Form' && $uielem_name == $postform) {
                  if ( !empty($this->variables['POSTDATA']) ) {
                    ...
                    foreach( $listFormFieldsArr as $listFormField) {
                      ...
                      $uielem->bind( array( array( $listFormField=>$bindArr) ) );
                    }
                    }
                    //var_dump( $listFormFieldsArr );
                    $uielem->bind( array( $this->variables['POSTDATA'] ) );
                    //var_dump( $uielem );
		...
		if ($action_screen)
		{
			...
			if ($this->type == 'nodes' && $template_name != 'none')
			{
                ...
                $breadcrumbEntries = array(
                		array('label'=>Language::getString('home'), 'href'=>$url),
                		array('label'=>$course->fullname, 'href'=>$url.'/course/view.php?id='.$course->id, 'last'=>false),
                		array('label'=>$app_desc, 'href'=>$nodeconfiguration[NodeConfKey::BASEURL], 'last'=>false),
                		array('label'=>$proc_desc, 'href'=>'', 'last'=>false),
                		array('label'=>$node_desc, 'href'=>'', 'last'=>true)
                		);
	...
	function get_submitted_data()
	{
		...
		foreach($_POST as $key=>$value)
		{
			if ($multi) {
				...
				$valuearray = $value + array('year' => 1970, 'month' => 1, 'day' => 1, 'hour' => 0, 'minute' => 0);
				$postdata[$key] = make_timestamp(
						$valuearray['year'],
						$valuearray['month'],
						$valuearray['day'],
						$valuearray['hour'],
		...
		array_walk($postdata, array($this, 'secureSuperGlobalPOST'));
...
//		var_dump($local_variables);
    ...
    function inst_addControl( $nodeconfiguration, $nodexml, $action_xmlnode, $instruction_xmlnode, $actionname,$instructionname,  &$variables, &$exit )
	{
      ...
      $this->current_controls[] = $control;
         ...
         * Send a mail
         *  @Param [Neon_Param] to
         *  @Param [Neon_Param] to_email
         *  @Param [Neon_Param] from
         *  @Param [Neon_Param] from_email
         *  @Param [Neon_Param] cc
         *  @Param [Neon_Param] bcc
         *  @Param [Neon_Param] reply_to
         *  @Param [Neon_Param] subject
         *  @Param [Neon_Param] body
         *  @Param [Neon_Param] charset
         *  @Param [Neon_Param] content_type
	...
	function inst_log( $nodeconfiguration, $nodexml, $action_xmlnode, $instruction_xmlnode, $actionname,$instructionname,  &$variables, &$exit )
	{
		...
		**/
		...
		$pars=array( 'table'=>'fw_log' ,'id_user'=>$KUINK_BRIDGE_CFG->auth->user->id, 'type'=>$type, 'application'=>$nodeconfiguration[NodeConfKey::APPLICATION], 'process'=>$nodeconfiguration[NodeConfKey::PROCESS], 'node'=>$nodeconfiguration[NodeConfKey::NODE], 'action'=>$action, 'log_key'=>(string)$key, 'timestamp'=>time(), 'message'=>$message);
		$log = $datasource->execute($pars);
	...
	function inst_call( $nodeconfiguration, $nodexml, $instruction_xmlnode, $instructionname,  &$variables, &$exit )
	{
		...
		foreach($paramsxml as $param)
		{
			...
			else if ($param->count() > 0)
			{
				...
				$param_value = (is_array($value)) ? $value : (string) $value;
			}
			else
				$param_value = (string)$param[0];
	...
	function inst_usermessage( $nodeconfiguration, $nodexml, $action_xmlnode, $instruction_xmlnode, $actionname,$instructionname,  &$variables, &$exit )
	{
		...
		if (! in_array($type,array('error','warning','information','success','exception')))
...
*/
	...
	function inst_execute( $nodeconfiguration, $nodexml, $action_xmlnode, $instruction_xmlnode, $actionname,$instructionname,  &$variables, &$exit )
	{
		...
		if ($paramsvar != "")
		{
			$var = $variables[$paramsvar];
			foreach($var as $key=>$value)
                            if ($key!='CHANGED') /** joao.patricio 16-01-2014 -> when passed params directly from postada and postdata has javascript variable changed **/
				$params[ "$key" ] = $value;
		}
...
function inst_dopdf( $nodeconfiguration, $nodexml, $action_xmlnode, $instruction_xmlnode, $actionname,$instructionname,  &$variables, &$exit ){
	...
	function inst_dopdfV2( $nodeconfiguration, $nodexml, $action_xmlnode, $instruction_xmlnode, $actionname,$instructionname,  &$variables, &$exit ){
		...
		$footer = $this->get_inst_attr($instruction_xmlnode, 'header', $variables, false, 'true');
