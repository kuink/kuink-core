...
class NodeManager {
	...
	public function getScreen($name) {
		return $this->getNodeObject($name, NodeType::NODE, NodeObject::SCREEN ,array('id','name'));
	}

	public function getScreenControls($name) {
		return $this->getNodeObject($name, NodeType::NODE, NodeObject::SCREEN.'[@name = "'.$name.'"]/*' ,array('id','name'));
	}
