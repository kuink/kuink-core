...
class Application {
	...
	private function loadApplicationDefinition() {
		...
		$this->xmlDefinition = simplexml_load_file($KUINK_CFG->appRoot.'apps/'.$appBase.'/' . $this->name . '/application.xml');
	}
...
//        //var_dump( $this->capabilities );
	...
	function run( $node=null, $functionName=null, $function_params=null) {
		...
		if ($KUINK_CFG->useNewDataAccessInfrastructure)
			//Setup framework dataSources
        	\Kuink\Core\DataSourceManager::setupDataSources( $this );
        else
			//Setup framework databases
        ...
        */
		...
		//$runNode = ($node == null) ? ProcessOrchestrator::getNodeToExecute($this->name, $this->roles, $this->defaultFlow) : $node;
	...
	private function getNodeConfiguration($node) {
		...
		//$url = new \moodle_url($baseUrl, $baseUrlParams);
