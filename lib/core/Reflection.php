...
class Reflection
{
	static function getBases() {
		...
		foreach ($apps as $app)
			$applications[] = array('name'=>(string)$app);
	...
	static function getApplicationProcesses( $application ) {
		...
		foreach ($procs as $proc)
          $processes[] = array('name'=>(string)$proc);
	...
	static function getProcessLibraries( $application, $process ) {
		...
		foreach ( $libs as $lib)
			$cleanLibs[] = array('name'=>(string)str_replace('.xml','',str_replace( $process.'_', '', $lib)));
	...
	static private function getNodeConfig( $nodeXml ) {
		...
		foreach ($configXml as $config)
			$nodeConfig[] = array('key'=>(string)$config['key'], 'value'=>(string)$config['value'] );
	...
	static private function getNodeScreens( $nodeXml ) {
		...
		foreach ( $dataXml as $data ) {
			...
			$nodeData[] = array( 'id'=>(string)$data['id'], 'doc'=>$dataDoc );
		}
	...
	static private function getNodePermissions( $nodeXml ) {
		...
		$nodePermissions = array('roles'=>$nodePermissionRoles, 'capabilities'=>$nodePermissionCapabilities );
	...
	static private function getNodeActions( $nodeXml ) {
		...
		foreach ( $actionsXml as $action ) {
			...
			$nodeActions[] = array( 'name'=>(string)$action['name'], 'type'=>(string)$action['type'], 'screen'=>(string)$action['screen'], 'doc'=>$actDoc, 'permissions'=>$permissions );
		}
	...
	static private function getNodeFunctions( $nodeXml, $application = null, $process=null, $node = null ) {
		...
		foreach ($fxXml as $fx) {
			...
			foreach ( $external as $outParam) {
				...
				$fxOutput[] = array( 'name' => $extName, 'type'=>$extType, 'doc'=>$extDoc);
			}
			...
			foreach ($errorsXml as $error) {
                $errors[(string)$error['code']] = array('code'=>(string)$error['code'], 'doc'=>(string)$error['doc']);
        ...
        $paramSign .= ($required == 'true') ? '<strong>*</strong>' : '';
			...
			$fx = array( 'name'=>(string)$fx['name'], 'library'=>$library, 'signature'=>$fxSignature, 'doc'=>$fxDoc, 'returnSignature' => $fxOutputSignature, 'return' => $fxOutput, 'returnType'=> $returnsType,'errors'=> $errors, 'errorsSignature' => $errorSignature, 'exceptionsSignature'=>$exceptionsSignature, 'params' => $functionParams, 'paramsSignature' => $fxParamsSignatureCompressed, 'scope'=>$fxScope, 'call'=>$call, 'permissions'=>$permissions,'fullQualifiedName'=>$fullQualifiedName);
			$nodeFunctions[] = $fx;				
		}
	...
	static private function getFunctionReturns($nodeXml, $function) {
		...
		foreach ($child as $ext) {
			...
			$extValue = array('name' => $name,
					'type' => $type,
					'doc' => $doc);
			$ret[] = $extValue;
		}
		
		return ( array( 'type'=>$retType, 'doc'=>$retDoc, 'external'=>$ret ) );
	}	
