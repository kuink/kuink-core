...
class Form extends Control {
	...
	function __construct( $nodeconfiguration, $xml_definition ) {
		...
		//$this->baseurl = $url->out(false).'&form='.$this->name;
	...
	function addRule( $params )
	{
      ...
      } else throw new \Exception($this->type.'->'.$this->name.'. addRule: invalid number of parameters.');

      if ($runat == 'client') {
        $clientCondition = $this->conditionToJavascript($ruleCondition, $this->name);
        $this->rulesClient[] = array( 'field'=>$id, 'attr'=>$ruleAttr, 'valueTrue'=>$ruleAttrValue, 'valueFalse'=>$oldAttrValue, 'condition'=>$clientCondition);
      }
	...
	function addFieldRule($fieldRule)
	{
		...
		if ($runat == 'client') {
			...
			if ($datasource!='') {
				...
				$this->rulesClient[] = array( 'field'=>$fieldId, 'attr'=>$rule['attr'], 'bindid'=>$rule['bindid'], 'bindvalue'=>$rule['bindvalue'], 'datasource'=>$clientDataSourceUrl, 'datasourceparams'=>$clientDataSourceParams  );
			} else
				$this->rulesClient[] = array( 'field'=>$fieldId, 'attr'=>$rule['attr'], 'valueTrue'=>$clientValueTrue, 'valueFalse'=>$clientValueFalse, 'condition'=>$clientCondition);
		}
		else if ($runat == 'server')
			;//$this->rulesServer[] = array( 'field'=>$id, 'attr'=>$ruleAttr, 'valueTrue'=>$ruleAttrValue, 'valueFalse'=>$oldAttrValue, 'condition'=>$condition);
	}
	...
	function applyRWX( $rwx ) {
		...
		if ( ($type == FormType::DATA) && ($rwx == 4) ) {
			$this->setProperty( array($this->name, FormProperty::FREEZE, 'true') );
			$this->setProperty( array($this->name, FormProperty::UNFREEZEBUTTONS, 'true') );
		}
	...
	function getCurrentData() {
	   ...
	   // (3)default data: if none of the above set the
        ...
        //var_dump($this->name.'_contextData');
        //var_dump($storedData);
        if (count($this->bind_data) > 0) {
            //var_dump('currentData::POSTDATA');
            return $this->bind_data[0];
        }
        if (count($storedData) > 0) {
            //var_dump('currentData::CONTEXT');
            return $storedData[0];
        }
        if (count($this->defaultData) > 0) {
            //var_dump('currentData::DEFAULT');
	...
	function addFormatter($formfield, $id)
	{
		...
		foreach ($f_xml as $child) {
			...
			if ($childname == 'Formatter') {
				$formatter_name = (string)$child['name'];
				$formatter_attributes = $child->attributes();
				$this->fieldFormatter[ $id ] = array( $formatter_name => $formatter_attributes);
			}
			//var_dump($this->fieldFormatter);
		}
		return;
	}
  ...
  //If this field has rules, then register them
			...
			if ($childname == 'Rule') {
              ...
              if ($ruleRunAt == 'client') {
                ...
                if ($datasource!='') {
                    ...
                    //Register API to the context
                    \Kuink\Core\ProcessOrchestrator::registerAPI($datasource);
                    $this->rulesClient[] = array( 'field'=>$id, 'attr'=>$ruleAttr, 'bindid'=>$attributes['bindid'], 'bindvalue'=>$attributes['bindvalue'], 'datasource'=>$clientDataSourceUrl, 'datasourceparams'=>$clientDataSourceParams  );
                } else
                    $this->rulesClient[] = array( 'field'=>$id, 'attr'=>$ruleAttr, 'valueTrue'=>$ruleAttrValue, 'valueFalse'=>$oldAttrValue, 'condition'=>$clientCondition);
              } else if ($ruleRunAt == 'server') {
                $this->rulesServer[] = array( 'field'=>$id, 'attr'=>$ruleAttr, 'valueTrue'=>$ruleAttrValue, 'valueFalse'=>$oldAttrValue, 'condition'=>$ruleCondition);
              } else
                throw new \Exception('Invalid runat value in rule');
			}
		}
	...
	function expandField( $formfield, $id, $type )
	{
		...
		if ($type == FieldType::TAB)
			$this->tabs[] = array(FieldProperty::ID=>$id, FieldProperty::LABEL=>(string)$attributes[FieldProperty::LABEL], 'columns'=>0 );
		...
		if ($type == FieldType::cSTATIC ) {
			...
			if ($options != null) {
				...
				foreach ($options as $key=>$value)
					$datasource[ $key ] = array( 'id'=>$key, 'name'=>$value);
	...
	function buildDynamicFields( $formfield, $fieldname )
	{
		...
		foreach($this->dynamic_fields as $dynamic_field)
		{
			...
			foreach ($dynamic_field as $key => $value)
				parent::setProperty( array($id, $key, $value) );

			//If there are options, then create a datasource and set it
			if (!is_null($options)) {
				$datasource = $dyn.'.'.$id;
				parent::setProperty( array($id, 'datasource', $datasource) );
				parent::setProperty( array($id, 'bindid', 'id') );
				parent::setProperty( array($id, 'bindvalue', 'value') );
			...
			if ($dynValue != '') {
				...
				$this->bind( array( array( $id => $dynValue ) ) );
			}
		}
	}
	...
	function buildActionGroup( $formfield, $fieldname ) {
		...
		foreach ($formfield->children() as $action) {
			...
			if ( $actionPermissions[ $name ] && $visible == 'true' ) {
				...
				$actionarray[$label] = array('url'=>$url, 'modal'=>$modal);
			}
		}
		...
		foreach($formfield->children() as $button)
		{
			...
			if ($visible == 'true') {
                ...
                if ( $event != '' || $runat=='client' || $actionPermissions[ $action ] ) {
                    ...
                    $skeleton = $attributes[FieldProperty::SKELETON];
                    $skin = $attributes[FieldProperty::SKIN];
                    $this->buttonActions[$id] = array('type' => $type , 'attributes'=>$attributes, 'skeleton'=>$skeleton , 'skin'=>$skin);
                }
			}
		}
	}
	...
	function bindData()
	{
       ...
       if (count($this->bind_data) == 0) {
            //No bind data
            //var_dump('NO POST DATA');
            if (count($storedData) > 0) {
                //Set context Data
                //var_dump('SET CONTEXT DATA');
		...
		//var_dump( $this->static_bind );
		//var_dump( $this->datasources );
		//var_dump( $this->dynamic_fields);
		...
		if ( $infer == 'true' ) {
			...
			foreach ($fields as $key=>$value ) {
				$this->addField( array( array('container'=>'_infer', 'id'=>$value, 'type'=>'Text', 'label'=>$value ) ) );
			}
				...
				foreach($bind_data as $key=>$value) {
					...
					if ($static_bind != '')
					{
						...
						if (isset($this->datasources[$datasourcename]))
						{
							...
							//kuink_mydebug($key.'.'.$value.'.', $datasource_value);
						//	if (empty($datasource_value))
								$new_value = $this->datasourceFindValue($datasource, $bindid, $bindvalue, $value);//	var_dump($datasource_value);
						//	else
						//		$new_value = (string)$datasource_value[$bindvalue];
								//kuink_mydebug( $key, $value.'::'.htmlentities(utf8_decode($new_value)) );
							...
							//$bindable_value = $datasource[];
                                                        //kuink_mydebug('Binding...'.$key, $bind_data[$key]);
						}
					}
					elseif (isset($this->static_fields[(string)$key])) {
						//var_dump($this->static_fields);
	...
	function display() {
		...
		$params['sRequiredFields'] = Core\Language::getString('requiredFields','framework', array( Core\Language::getString('requiredString') ));
        ...
        $personTimeZoneOffset = $dateTimeLib->getTzOffset(array(0=>$neon_user['timezone']));
