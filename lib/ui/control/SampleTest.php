 ...
 */
class SampleTest extends Control {
	/*
	...
	function display() {
		...
		$icon = ( string ) $this->getProperty ( '', 'icon', true, null, $this->xml_definition );
		...
		$valueXml = $this->getInnerElement('Value', true, $this->xml_definition);
		...
		$valueAttr = ( string ) $this->getProperty('', 'attribute', true, null, $valueXml);
		...
		$value = isset($data[$valueAttr]) ? (string) $data[$valueAttr] : '';
