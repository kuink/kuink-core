...
class Grid extends Control {
    ...
    function __construct($nodeconfiguration, $xml_definition) {
        ...
        if (isset($_GET[$this->name . '_'.GridContextVariables::PAGE_SORT])) {
        	...
        	if ($sortSplit[1] != 'rem')
        		$this->sort[$sortSplit[0]] = array('column'=>$sortSplit[0], 'sort'=>$sortSplit[1]);
        	else
        ...
        //\Kuink\Core\ProcessOrchestrator::setProcessVariable('__grid_'.$this->name, 'page', $this->page);
        //Getting the pagesize to display
        //$currentStoredPageSize = \Kuink\Core\ProcessOrchestrator::getProcessVariable('__grid_'.$this->name, 'pagesize');
    ...
    function getSort($params) {
      ...
      $useDefaultSort = (!isset($this->sort));
        ...
        if (($useDefaultSort) && ($defaultSort != '')) {
          ...
          $this->sort[$count] = array('column'=>$count, 'sort'=>$defaultSort);
        	$this->setContextVariable(GridContextVariables::PAGE_SORT, $this->sort);
        }
    ...
    function buildDynamicFields() {
        ...
        foreach ($this->dynamicColumns as $dynamicColumn) {
            ...
            foreach ($dynamicColumn as $key => $value)
                parent::setProperty(array($name, $key, $value));

            //If there are options, then create a datasource and set it
            if (!is_null($options)) {
                $datasource = $dyn . '.' . $id;
                parent::setProperty(array($name, 'datasource', $datasource));
                parent::setProperty(array($name, 'bindid', 'id'));
                parent::setProperty(array($name, 'bindvalue', 'value'));
...
//var_dump($dynFormatters);
            ...
            if (count($column->children()) > 0) {
                ...
                foreach ($formatters as $colformatter) {
                    ...
                    if ($block == 'Rule') {
                        ...
                        $ruleCondition = array('condition' => $ruleAttrCondition, 'capability' => $ruleAttrCapability, 'value' => $ruleAttrValue);
                        ...
                        $colformatter_collection[] = array('formatter' => $formatter, 'attributes' => $attributes);
                    }
                }
            }
            $colformatter = array();
    ...
    private function bindData() {
        ...
        if ($this->pivot == 'true') {
            ...
            foreach ($this->bind_data as $data) {
                $this->bind_data[$count++] = $utilsLib->pivotTable(array((array) $data, $this->pivotlines, $this->pivotcols, $this->pivotdata));
            }
        }
            ...
            foreach ($records as $record) {
                ...
                foreach ($this->tablecolumns as $column) {
                    ...
                    foreach ($colRules as $attrName => $ruleCondition) {
                        ...
                        if ($condCapability != '' && $capabilityValue == 1) {
                            $colAttributes[$attrName] = $condAttrValue;
                        }
                    ...
                    if ($colActionName != '' && $colActionValue != '')
                        if (!empty($action_permissions[$colActionName])) {
                            //The user has permission
                            $colActionUrl = \Kuink\Core\Tools::setUrlParams($baseurl, array('action' => $colActionName, 'actionvalue' => $colActionValueBind));
                ...
                if ($this->hasactions) {
                    ...
                    foreach ($this->actions as $action) {
                        ...
                        if (!empty($action_permissions[$actionname])) {
                            ...
                            $location = \Kuink\Core\Tools::setUrlParams($baseurl, array('action' => $actionname, 'actionvalue' => $actionvalue));
                            //print($location);
                            ...
                            if ($cond_fieldvalue == $action_condvalue || $conditionTrue == 1) {
                                ...
                                if ($confirm_label != 'false') {
                                    ...
                                    $this->tableConfirmActions[] = array('actionName' => $actionname, 'actionValue' => $actionvalue, 'href' => $location, 'confirmText' => $confirm_text, 'target' => $target);
    ...
    private function getActionTypeData($actionType) {
      ...
      switch($actionType) {
        case 'download':
          $actionTypeData = array('icon'=>'cloud-download', 'icon-color'=>'icon-white', 'icon-only'=>false, 'class'=>'btn btn-success');
          break;
        case 'execute':
         	$actionTypeData = array('icon'=>'play', 'icon-color'=>'icon-white', 'icon-only'=>false, 'class'=>'btn btn-primary');
         	break;
        default:
          $actionTypeData = array('icon'=>'', 'icon-color'=>'icon-black', 'icon-only'=>true, 'class'=>'');
          break;
      }
     ...
     * @param unknown_type $data - dataset with row data
     * @param unknown_type $params - params to the formatter
     */
    /*
    ...
    private function displayChart($view) {
        ...
        $params['properties'] = array('width' => $width, 'height' => $height, 'bgcolor' => $bgColor);
    ...
    public function export($type) {
        ...
        foreach ($this->tableheaders as $header) {
            $fixed_headers[] = '"' . str_replace('"', '""', utf8_decode(html_entity_decode(strip_tags($header)))) . '"';
        }
        ...
        foreach ($this->databind as $data) {
            foreach ($data as $row_key => $row_value) {
                ...
                foreach ($row_value as $key => $value) {
                    $fixed_data[] = '"' . str_replace('"', '""', utf8_decode(html_entity_decode(strip_tags($value['value'])))) . '"';
                }
