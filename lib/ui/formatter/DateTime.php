...
class DateTime extends Formatter {
	 ...
	 * @category time
	 * @param int $date
	 *        	the timestamp in UTC, as obtained from the database.
	 * @param string $format
	 *        	strftime format. You should probably get this using
	 *        	get_string('strftime...', 'langconfig');
	 * @param int|float|string $timezone
	 *        	by default, uses the user's time zone. if numeric and
	 *        	not 99 then daylight saving will not be added.
	 *        	{@link http://docs.moodle.org/dev/Time_API#Timezone}
	 * @param bool $fixday
	 *        	If true (default) then the leading zero from %d is removed.
	...
	private function neon_userdate($date, $format = '', $timezone = 99, $fixday = true, $fixhour = true) {
		...
		if (! empty ( $KUINK_CFG->nofixday )) { // Config.php can force %d not to be fixed.
			$fixday = false;
		} else if ($fixday) {
		...
		if (abs ( $timezone ) > 13) { // / Server time
			...
			if ($fixday) {
				$daystring = ltrim ( str_replace ( array (
						' 0',
						' ' 
				), '', strftime ( ' %d', $date ) ) );
			...
			if ($fixhour) {
				$hourstring = ltrim ( str_replace ( array (
						' 0',
						' ' 
				), '', strftime ( ' %I', $date ) ) );
