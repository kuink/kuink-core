...
class DateTimeLib
{
    ...
    */
     ...
     * @param String $params 0 : Timezone string
    ...
    **/
    function getComponents($params){
        ...
        if(isset($params[1])) {
          $format = $params[1];
          $hourArray = array($timestamp, $format);
        } else {
          $hourArray = array($timestamp);
        }
        $out['day'] = $this->getDay(array($timestamp));
        $out['month'] = $this->getMonth(array($timestamp));
        $out['year'] = $this->getYear(array($timestamp));
        $out['hour'] = $this->getHour($hourArray);
        $out['minute'] = $this->getMinutes(array($timestamp));
        $out['second'] = $this->getSeconds(array($timestamp));
        return $out;
    }
    ...
    function splitDateTime($params){
    	...
    	return array("hour" => $parts[0], "minute" => $parts[1], "second" => $parts[2]);
    }
    ...
    **/
    function getLastDayOfMonth($params=null){
        ...
        $components = $this->getComponents(array($timestamp));
